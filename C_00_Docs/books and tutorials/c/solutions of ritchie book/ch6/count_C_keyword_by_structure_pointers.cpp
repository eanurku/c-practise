...
struct key * binary_search(char *element,struct key *element_list,int size)
{
    struct key *start=element_list,*end=element_list+size-1,*middle;
    
    while(start<=end) {

       middle=start+(end-start)/2;

       if(strcmp(element,middle->word)<0)
                    end=middle-1;
       else if(strcmp(element,middle->word)>0)
                    start=middle+1;
       else
                    return middle; // 1 for match and 0 for mismatch 
...
int main(int argc,char *argv[])
{
  ...
  struct key *n;
    ...
    while(word_len=strlen( word=getword()))
       ...
       if((n=binary_search(word,keywords,NKEYS))!=NULL)
          n->count++;
          
    for(n=keywords;n<keywords+NKEYS;n++)
    if(n->count)
       printf("%s=%d\n",n->word,n->count);
  free(word);
  getch();
