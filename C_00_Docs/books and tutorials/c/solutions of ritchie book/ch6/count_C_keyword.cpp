...
int binary_search(char *element,char *element_list[],int size)
{
    ...
    while(start<=end) {
       ...
       if(strcmp(element,element_list[middle])<0)
                    end=middle-1;
       else if(strcmp(element,element_list[middle])>0)
                    start=middle+1;
       else
