...
{ int i;
  ...
  for(i=0;i<HASH_SIZE;i++)     
     if(ptr=hash_array[i])
     {  printf("hash_array[%d] ",i);
        for(;ptr;ptr=ptr->next)  
            printf("%s/%d ",ptr->word,ptr->count);
        printf("\n");
     }
} 
void undef(char *word,struct node *hash_array[])
{  
     typedef struct node * x;
     x ptr,prev;
     
     
       ptr=hash_array[hash_function(word)];
       //match at start
       if(!strcmp(ptr->word,word))
        {   
            hash_array[hash_function(word)]=ptr->next;
            
            return ;
        }
       //for match  in  middle(minimum 3 node)
       prev=ptr;
       for(ptr=ptr->next;ptr && ptr->next;ptr=ptr->next)
       {  
            if(!strcmp(ptr->word,word))              
              {   prev->next=ptr->next;
                    return ;
